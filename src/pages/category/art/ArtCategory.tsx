...
import { artImagesByFilename } from '@/data/images';
import { artFilters } from './config';
...
interface ArtProduct {
  ...
  attributes: {
    ...
    style?: string[];
    size?: string;
    dimensions?: { width?: number; height?: number; diameter?: number; unit: string };
  };
  tags: string[];
  metadata: {
    featured?: boolean;
...
export default function ArtCategory() {
  ...
  const products = artData as ArtProduct[];
  ...
  useEffect(() => {
    ...
    if (size) {
      initialFilters['size'] = size.split(',');
    }
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilters['size']?.length > 0) {
      filtered = filtered.filter(p =>
        selectedFilters['size'].includes(p.attributes.size || '')
      );
    }
          ...
          <aside className="w-[345px] flex-shrink-0 pr-6 py-6 pl-4 border-r border-gray-100">
            <div className="sticky top-24">
              ...
              <nav className="text-xs text-gray-500 mb-4">
                ...
                <span className="text-gray-900">ART & MIRRORS</span>
              </nav>
                  ...
                  >
                    <Link to={`/product/${product.id}`} className="group block">
                      ...
                      <div className="relative aspect-square bg-gray-50 mb-3 overflow-hidden">
                        <img
                          src={artImagesByFilename[product.primaryImage]}
                          alt={product.name}
