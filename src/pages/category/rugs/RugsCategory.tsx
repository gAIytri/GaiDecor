...
import { rugsImagesByFilename } from '@/data/images';
import { rugsFilters } from './config';

// Rug product type with size as array
type RugProduct = Omit<Product, 'attributes'> & {
  attributes: Omit<Product['attributes'], 'size'> & {
    size?: string[];
  };
};

export default function RugsCategory() {
  const [searchParams, setSearchParams] = useSearchParams();
  const [expandedFilters, setExpandedFilters] = useState<string[]>([]);
  const [selectedFilters, setSelectedFilters] = useState<Record<string, string[]>>({});
  const [sortBy, setSortBy] = useState<string>('popularity');

  const products = rugsData as RugProduct[];
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilters['color']?.length > 0) {
      filtered = filtered.filter(p =>
        p.attributes.color?.some((c: string) =>
    ...
    if (selectedFilters['material']?.length > 0) {
      filtered = filtered.filter(p =>
        p.attributes.material?.some((m: string) =>
    ...
    if (selectedFilters['style']?.length > 0) {
      filtered = filtered.filter(p =>
        p.attributes.style?.some((s: string) =>
          ...
          <aside className="w-[345px] flex-shrink-0 pr-6 py-6 pl-4 border-r border-gray-100">
            <div className="sticky top-24">
              ...
              <nav className="text-xs text-gray-500 mb-4">
                ...
                <span className="text-gray-900">RUGS</span>
              </nav>

              {rugsFilters.map((filter) => (
                  ...
                  >
                    <Link to={`/product/${product.id}`} className="group block">
                      ...
                      <div className="relative aspect-square bg-gray-50 mb-3 overflow-hidden">
                        <img
                          src={rugsImagesByFilename[product.primaryImage]}
                          alt={product.name}
                      ...
                      {product.attributes.color && product.attributes.color.length > 0 && (
                        <div className="flex gap-1 mb-2">
                          {product.attributes.color.slice(0, 4).map((color: string, i: number) => (
                            <div
                              key={i}
