...
import { beddingImagesByFilename } from '@/data/images';
import { beddingFilters } from './config';

// Bedding product type
interface BeddingProduct {
  id: string;
  name: string;
  description?: string;
  price: number;
...
export default function BeddingCategory() {
  ...
  const products = beddingData as BeddingProduct[];
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedFilters['room']?.length > 0) {
      filtered = filtered.filter(p =>
        p.attributes.room?.some((r: string) =>
          selectedFilters['room'].includes(r)
        )
      );
    }
                  ...
                  >
                    <Link to={`/product/${product.id}`} className="group block">
                      ...
                      <div className="relative aspect-square bg-gray-50 mb-3 overflow-hidden">
                        <img
                          src={beddingImagesByFilename[product.primaryImage]}
                          alt={product.name}
